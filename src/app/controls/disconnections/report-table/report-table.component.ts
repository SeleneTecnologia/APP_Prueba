...
import { EafDTO, EafService } from '@services/eaf.service';
...
@Component({
  ...
  standalone: true,
  imports: [TableModule, CommonModule, ConfirmDialogModule, ConfirmPopupComponent],
...
export class ReportTableComponent implements OnInit, OnDestroy {
      ...
      .subscribe({
        next: (data) => {
          ...
          this.eafs = data.eafs.map(eaf => ({
            ...eaf,
            entryTimestamp: this.datePipe.transform(eaf.entryTimestamp, 'dd-MM-yyyy HH:mm') + '' // Formatear la fecha
          }));
  ...
  navigateToDetail(eafId: number): void {
    this.router.navigate([`desconexion`, eafId], { relativeTo: this.route });
  }
  ...
  rejectDelete(){
    this.messageService.add({ severity: 'error', summary: 'Error', detail: 'El registro no se eliminó' });
  }
